...
def load_data(uploaded_file):
    try:
        ...
        # Calculate ROI if columns exist
...
if not df.empty:
    ...
    st.markdown("Analyze campaign performance with **interactive filters, charts, and KPIs**.")
    ...
    # ROI Trend (if Campaign ID is available)
    if 'Campaign ID' in filtered_df.columns and 'ROI' in filtered_df.columns:
        ...
        sns.lineplot(data=df_sorted, x='Campaign ID', y='ROI', marker='o', hue='Platform' if 'Platform' in df.columns else None, ax=ax)
    ...
    st.warning("⚠️ Could not load data. Please check your file.")
